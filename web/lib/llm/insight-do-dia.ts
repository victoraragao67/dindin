...
export async function getInsightDoDia(
  ...
  // 2. Gera redação (LLM → guardrail → fallback; nunca propaga erro para a UI)
  ...
  // Guardrail: valida saída LLM antes de usar
  ...
  let origem: 'llm' | 'fallback' = 'fallback'
  let resumo: string
  ...
  if (llm?.resumo) {
    ...
    if (check.ok) {
      ...
      console.warn('[insight-do-dia] guardrail rejeitou LLM:', check.motivo)
      resumo = gerarFallbackTemplate(preditiva, diasRestantes).slice(0, 280)
    }
  } else {
    resumo = gerarFallbackTemplate(preditiva, diasRestantes).slice(0, 280)
  }
